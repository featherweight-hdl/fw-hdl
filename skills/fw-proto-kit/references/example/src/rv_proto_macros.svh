...
// ----------------------------------------------------------------------
// Implementation-template macros shipped by the rv APIs. EVERY API ships one,
...
//
// `FW_RV_INITIATOR_IMP(T, IMP, NAME) -- declares an export member NAME whose
//   send() redirects to IMP's NAME``_send(). The macro call needs a trailing `;`.
// `FW_RV_TARGET_IMP(T, IMP, NAME)    -- declares an export member NAME whose
//   put() redirects to IMP's NAME``_put().
// `FW_RV_MONITOR_IMP(T, IMP, NAME)   -- declares an export member NAME whose
//   observe() redirects to IMP's NAME``_observe(). observe() is a FUNCTION
//   (non-blocking) -- monitor APIs may not block.
// ----------------------------------------------------------------------
